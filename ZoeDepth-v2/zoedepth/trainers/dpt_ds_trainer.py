...
from zoedepth.trainers.loss import GradL1Loss, SILogLoss, ScaleAndShiftInvariantLoss, MultiscaleGradientLoss, DeepSupervisedLoss
...
class Trainer(BaseTrainer):
    ...
    def train_on_batch(self, batch, train_step):
        ...
        with amp.autocast(enabled=self.config.use_amp):
            ...
            l_deep_sup = self.deepsup_loss(deep_sup_features, depths_gt, mask=mask)
            loss = loss + self.config.w_deep_sup * l_deep_sup
            losses[self.deepsup_loss.name] = l_deep_sup
