...
class DPT(DepthModel):
    ...
    def forward(self, x, return_final_centers=False, denorm=False, return_probs=False, **kwargs):
        ...
        rel_depth = self.core(x, denorm=denorm, return_rel_depth=True) # no need fetch features here
        output = dict()
        output['rel_depth'] = rel_depth # currently learn to predict disparity
        return output
    ...
    def build(midas_model_type="DPT_BEiT_L_384", pretrained_resource=None, use_pretrained_midas=False, train_midas=False, freeze_midas_bn=True, **kwargs):
        core = MidasCore.build(midas_model_type=midas_model_type, use_pretrained_midas=use_pretrained_midas,
                               train_midas=train_midas, fetch_features=False, freeze_bn=freeze_midas_bn, **kwargs)
